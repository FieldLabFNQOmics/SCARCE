...
# helper: return env var as string or NULL (never TRUE/FALSE)
.env_or_null <- function(key) {
  val <- Sys.getenv(key, "")
  if (nzchar(val)) val else NULL
}

# helper: split an env var on spaces (or commas), or return character(0)
.env_split_or_empty <- function(key, split = " +") {
  val <- Sys.getenv(key, "")
  if (nzchar(val)) strsplit(val, split)[[1]] else character()
}
...
.get_cfg <- function(){
  ...
  if (is.null(cfg)) {
    vep_config(
      ...
      vep_path   = .env_or_null("MYVEP_BIN"),         # <- string or NULL
      conda_env  = .env_or_null("MYVEP_CONDA_ENV"),   # <- string or NULL
      image      = .env_or_null("MYVEP_IMAGE"),       # <- string or NULL
      bind       = {
        b <- .env_split_or_empty("MYVEP_BIND", split = ",")
        if (length(b)) b else NULL
      },
      cache_dir  = Sys.getenv("VEP_CACHE", "~/.vep"),
      fasta      = .env_or_null("MYVEP_FASTA"),       # <- string or NULL
      species    = Sys.getenv("MYVEP_SPECIES", "homo_sapiens"),
      assembly   = Sys.getenv("MYVEP_ASSEMBLY", "GRCh38"),
      threads    = {
        th <- suppressWarnings(as.integer(Sys.getenv("MYVEP_THREADS", "1")))
        if (is.na(th) || th < 1L) 1L else th
      },
      extra_args = {
        ea <- .env_split_or_empty("MYVEP_EXTRA", split = " +")
        if (length(ea)) ea else character()
      }
    )
  }
